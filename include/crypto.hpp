...
public:
	...
	public:
		static std::string encode(const std::string &ascii) {
			...
			auto base64_length = static_cast<std::size_t>(round(
					4 * ceil(static_cast<double>(ascii.size()) / 3.0)));
			base64.resize(base64_length);
			bptr->length = 0;
			bptr->max = base64_length + 1;
			bptr->data = &base64[0];

			if (BIO_write(b64, &ascii[0], static_cast<int>(ascii.size())) <= 0
					|| BIO_flush(b64) <= 0)
				base64.clear();
...
// TODO: Remove in 2020
			...
			auto decoded_length = BIO_read(bio, &ascii[0],
	...
	static std::string md5(const std::string &input, std::size_t iterations = 1)
			{
		std::string hash;
	...
	static std::string md5(std::istream &stream, std::size_t iterations = 1)
			{
		MD5_CTX context;
		MD5_Init(&context);
		...
		while ((read_length = stream.read(&buffer[0], buffer_size).gcount()) > 0)
			MD5_Update(&context, buffer.data(),
	...
	static std::string sha1(std::istream &stream, std::size_t iterations = 1)
			{
		SHA_CTX context;
		SHA1_Init(&context);
		std::streamsize read_length;
		std::vector<char> buffer(buffer_size);
		while ((read_length = stream.read(&buffer[0], buffer_size).gcount()) > 0)
			SHA1_Update(&context, buffer.data(),
					static_cast<std::size_t>(read_length));
		std::string hash;
		hash.resize(160 / 8);
	...
	static std::string sha256(const std::string &input, std::size_t iterations =
			1) {
		std::string hash;
		...
		for (std::size_t c = 1; c < iterations; ++c)
			SHA256(reinterpret_cast<const unsigned char *>(&hash[0]),
					hash.size(), reinterpret_cast<unsigned char *>(&hash[0]));

		return hash;
	}

	static std::string sha256(std::istream &stream, std::size_t iterations = 1)
			{
		SHA256_CTX context;
		SHA256_Init(&context);
		std::streamsize read_length;
	...
	static std::string sha512(const std::string &input, std::size_t iterations =
			1) {
		std::string hash;
	...
	static std::string sha512(std::istream &stream, std::size_t iterations = 1)
			{
		SHA512_CTX context;
		SHA512_Init(&context);
		std::streamsize read_length;
	...
	static std::string pbkdf2(const std::string &password,
			const std::string &salt, int iterations, int key_size) {
		std::string key;
