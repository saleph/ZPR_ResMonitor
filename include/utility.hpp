		...
		for (auto &chr : value) {
			if (chr == ' ')
				result += '+';
					...
					|| chr == '=' || chr == '?' || chr == '@' || chr == '['
					|| chr == ']')
				result += std::string("%") + hex_chars[chr >> 4]
						+ hex_chars[chr & 15];
			else
				result += chr;
		}
	...
	static std::string decode(const std::string &value) {
		...
		for (std::size_t i = 0; i < value.size(); ++i) {
			...
			if (chr == '%' && i + 2 < value.size()) {
				...
				auto decoded_chr = static_cast<char>(std::strtol(hex.c_str(),
						nullptr, 16));
				result += decoded_chr;
				i += 2;
			} else if (chr == '+')
...
public:
	...
	static CaseInsensitiveMultimap parse(const std::string &query_string)
		...
		if (name_pos < query_string.size()) {
			...
			if (!name.empty()) {
				auto value =
						value_pos >= query_string.size() ?
								std::string() : query_string.substr(value_pos);
				result.emplace(std::move(name), Percent::decode(value));
			}
		}
	...
	public:
		...
		public:
			...
			static CaseInsensitiveMultimap parse(const std::string &str) {
				...
				if (name_start_pos != std::string::npos) {
					if (name_end_pos == std::string::npos)
						result.emplace(str.substr(name_start_pos),
								std::string());
...
public:
	...
	static bool parse(std::istream &stream, std::string &method,
			std::string &path, std::string &query_string, std::string &version,
...
public:
	...
	static bool parse(std::istream &stream, std::string &version,
			std::string &status_code, CaseInsensitiveMultimap &header)
					{
		header.clear();
		std::string line;
		getline(stream, line);
